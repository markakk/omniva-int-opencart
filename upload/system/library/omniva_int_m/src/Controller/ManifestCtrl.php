...
class ManifestCtrl
{
    public static function list($db, $offset = 0, $limit = 30)
    {
        $sql = $db->query(
            "
            SELECT api_cart_id, COUNT(DISTINCT api_shipment_id) as total_orders FROM " . DB_PREFIX . "omniva_int_m_order_api
            WHERE `canceled` = 0
    ...
    {
        $sql = $db->query("
            SELECT COUNT(DISTINCT api_cart_id) as `total` FROM " . DB_PREFIX . "omniva_int_m_order_api
            WHERE canceled = 0
        ");
