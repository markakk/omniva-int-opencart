...
class Helper
{
    ...
    public static function saveSettings($db, $data)
    {
        foreach ($data as $key => $value) {
            $query = $db->query("SELECT setting_id FROM `" . DB_PREFIX . "setting` WHERE `code` = 'omniva_int_m' AND `key` = '" . $db->escape($key) . "'");
            if ($query->num_rows) {
                $id = $query->row['setting_id'];
                $db->query("UPDATE " . DB_PREFIX . "setting SET `value` = '" . $db->escape($value) . "', serialized = '0' WHERE `setting_id` = '$id'");
            } else {
                $db->query("INSERT INTO `" . DB_PREFIX . "setting` SET store_id = '0', `code` = 'omniva_int_m', `key` = '$key', `value` = '" . $db->escape($value) . "'");
            }
        }
    }
    ...
    {
        try {
            if ($session && isset($session->data['omniva_int_m_services']) && $session->data['omniva_int_m_services']['valid_until'] > time()) {
                return Service::fromArray(self::base64Decode($session->data['omniva_int_m_services']['services']));
            }
            ...
            if ($session) {
                $session->data['omniva_int_m_services'] = [
    ...
    public static function hasGitUpdate()
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_USERAGENT => 'OMNIVA_INT_M_VERSION_CHECK_v1.0',
        ));
    ...
    public static function getReceiver($order_data, $country, $type, $terminal_data = null)
    {
        ...
        if (!empty($order_data['omniva_int_m_hs_code'])) {
            $receiver->setHsCode($order_data['omniva_int_m_hs_code']);
        }
