...
class ModelExtensionShippingOmnivaIntM extends Model
{
    public function getQuote($address)
    {
        ...
        $this->session->data['omniva_int_m_cart_offers'] = [];
    ...
    {
        if (!isset($this->session->data['omniva_int_m_cart_offers'])) {
            return;
        }

        $order_id = $this->session->data['order_id'];
        $cart_offers = $this->session->data['omniva_int_m_cart_offers'];
        $shipping_method = $this->session->data['shipping_method'];

        $shipping_method = str_replace('omniva_int_m.', '', $shipping_method['code']);
        ...
        if (isset($this->session->data['omniva_int_m_cart_selected_terminal'][$shipping_method])) {
            $terminal_data = $this->session->data['omniva_int_m_cart_selected_terminal'][$shipping_method];
            $terminal_id = "'" . $terminal_data['terminal_id'] . "'"; // sql ready
            unset($this->session->data['omniva_int_m_cart_selected_terminal']);
        }
        ...
        $this->db->query("
            INSERT INTO " . DB_PREFIX . "omniva_int_m_order
        ...
        unset($this->session->data['omniva_int_m_cart_offers']);
    }
    ...
    public function updateOrderInformation($order_id)
    {
        ...
        if (!isset($this->session->data['omniva_int_m_cart_offers']) || $order_id <= 0) {
            return;
        }

        // check if its an existing omniva_int order
        $is_order = (bool) $this->db->query(
            "
            SELECT order_id FROM " . DB_PREFIX . "omniva_int_m_order
    ...
    public function getFrontData()
    {
        ...
        $data['omniva_int_m_api_url'] = (bool) $this->config->get(Params::PREFIX . 'api_test_mode') ? Params::API_URL_TEST : Params::API_URL;
        $data['omniva_int_m_api_images_url'] = (bool) $this->config->get(Params::PREFIX . 'api_test_mode') ? Params::API_IMAGES_URL_TEST : Params::API_IMAGES_URL;
