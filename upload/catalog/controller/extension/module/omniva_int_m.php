...
class ControllerExtensionModuleOmnivaIntM extends Controller
{
    ...
    private function getFrontCurrentData()
    {
        ...
        $shipping_method = $this->session->data['shipping_method'] ?? null;
        ...
        return [
            ...
            'address' => $address,
            'selection' => $this->session->data['omniva_int_m_cart_selected_terminal'] ?? []
        ];
    }

    private function selectTerminal()
    {
        if (!isset($this->request->post['terminal_id']) || !isset($this->request->post['omniva_int_m_option'])) {
            return [
        ...
        if (
            strpos($this->request->post['omniva_int_m_option'], 'omniva_int_m.terminal_') !== 0 ||
            strlen($this->request->post['omniva_int_m_option']) > 100
        ) {
            return [
        ...
        $option = $this->db->escape(strip_tags($this->request->post['omniva_int_m_option']));
        $option = explode('.', $option)[1];

        if (!isset($this->session->data['omniva_int_m_cart_selected_terminal'])) {
            $this->session->data['omniva_int_m_cart_selected_terminal'] = [];
        }

        $this->session->data['omniva_int_m_cart_selected_terminal'][$option] = [
