...
require_once(DIR_SYSTEM . 'library/omniva_int_m/vendor/autoload.php');

use Mijora\OmnivaIntOpencart\Controller\ManifestCtrl;
...
use Mijora\OmnivaIntOpencart\Model\ShippingOptionCountry;
use Mijora\OmnivaIntOpencart\Params;
use OmnivaApi\API;
use OmnivaApi\Order;

class ControllerExtensionShippingOmnivaIntM extends Controller
{
    private $error = array();
    ...
    public function install()
    {
        $sql_array = [
            "
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "omniva_int_m_country` (
            ...
            ",
            "
            CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "omniva_int_m_option` (
    ...
    public function uninstall()
    {
        ...
        $this->model_setting_setting->deleteSetting('omniva_int_m');

        $sql_array = [
            "DROP TABLE IF EXISTS `" . DB_PREFIX . "omniva_int_m_country`",
    ...
    public function index()
    {
        ...
        if (isset($this->request->get['fixdb']) && $this->validate()) {
            $this->fixDb();
            $this->response->redirect($this->url->link('extension/shipping/omniva_int_m', $this->getUserToken(), true));
        }
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if (isset($this->request->post['api_settings_update'])) {
                ...
                $this->session->data['success'] = $this->language->get('omniva_int_m_msg_setting_saved');
                $current_tab = 'api';
            }
            ...
            $this->response->redirect($this->url->link('extension/shipping/omniva_int_m', $this->getUserToken() . '&tab=' . $current_tab, true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/shipping/omniva_int_m', $this->getUserToken(), true)
        );

        $data['action'] = $this->url->link('extension/shipping/omniva_int_m', $this->getUserToken(), true);
        ...
        $data['ajax_url'] = 'index.php?route=extension/shipping/omniva_int_m/ajax&' . $this->getUserToken();
        ...
        $data[Params::PREFIX . 'db_fix_url'] = $this->url->link('extension/shipping/omniva_int_m', $this->getUserToken() . '&fixdb', true);
        ...
        $data[Params::PREFIX . 'xml_fix_url'] = $this->url->link('extension/shipping/omniva_int_m', $this->getUserToken() . '&fixxml', true);
        ...
        $this->response->setOutput($this->load->view('extension/shipping/omniva_int_m/settings', $data));
    }
    ...
    {
        if (!$this->user->hasPermission('modify', 'extension/shipping/omniva_int_m')) {
    ...
    private function saveShippingOption()
    {
        ...
        $shipping_option->enabled = $enabled;
    ...
    {
        return $this->load->view('extension/shipping/omniva_int_m/pagination', [
    ...
    private function cancelShipment()
    {
        ...
        $this->db->query("
            UPDATE " . DB_PREFIX . "omniva_int_m_order_api
            SET canceled = 1
    ...
    public function manifest()
    {
        ...
        $this->response->setOutput($this->load->view('extension/shipping/omniva_int_m/manifest', $data));
    }
