var OMNIVA_INT_M_COMMON = {
    ...
    showLoadingOverlay: function (shouldShow, target) {
        ...
        const hasLoaderElement = target.querySelector('.omniva_int_m_loader');

        if (shouldShow && !hasLoaderElement) {
            let loaderEl = document.createElement('div');
            loaderEl.classList.add('omniva_int_m_loader');
            loaderEl.innerHTML = OMNIVA_INT_M_COMMON.loaderHtml;
    ...
    alert: function ({ message, onClose }) {
        ...
        let dialog = document.querySelector('dialog[data-omnivaintm-alert-dialog]');
        if (!dialog) {
            dialog = document.createElement('dialog');
            dialog.dataset.omnivaintmAlertDialog = true;
    ...
    confirm: function ({ message, accept, cancel }) {
        ...
        let dialog = document.querySelector('dialog[data-omnivaintm-confirm-dialog]');
        if (!dialog) {
            dialog = document.createElement('dialog');
            dialog.dataset.omnivaintmConfirmDialog = true;
            dialog.style.padding = 0;
            let dialogHtml = `
                <form method="dialog" class="panel panel-default" style="margin: 0;">
                    <div class="panel-body" data-alert-message></div>
